...
if (isset($_POST['doimatkhau'])){
    ...
    if(isset($_SESSION['id_admin'])) {
        ...
        $sql_admin = "SELECT * FROM tbl_admin WHERE id_admin = '".$id_admin."' AND matkhau ='".$matkhau_cu."' LIMIT 1";
        $result_admin = mysqli_query($mysqli, $sql_admin);
        $count_admin = mysqli_num_rows($result_admin);

        if ($count_admin > 0) {
            $sql_update_admin = "UPDATE tbl_admin SET matkhau ='".$matkhau_moi."' WHERE id_admin = '".$id_admin."'";
